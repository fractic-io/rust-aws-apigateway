    ...
    ($validator:ident, $func:ident, $request_data_type:ident) => {
        async fn __handler(
            ...
            let metadata = match $crate::parse_request_metadata(&event.payload) {
                Ok(m) => m,
                Err(e) => return $crate::build_error(e),
            };
            match $crate::parse_request_data::<$request_data_type>(&event.payload) {
                Ok(obj) => match $validator(&obj, metadata) {
                    Ok(_) => match $func(obj).await {
                        Ok(result) => $crate::build_result(result),
                        Err(func_error) => $crate::build_error(func_error),
                    },
                    Err(validation_error) => $crate::build_error(validation_error),
                },
                Err(request_parsing_error) => $crate::build_error(request_parsing_error),
            }
        }
